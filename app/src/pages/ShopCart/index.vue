...
export default {
  ...
  methods: {
    ...
    handler: throttle(async function (type, disNum, cart) {
      //type:区分三个元素
      //disNum: + 变化量（1） - 变化量（-1） input变化量（$event.target.value-之前个数）
      //cartId:哪个产品的id
      switch (type) {
        case "plus":
          disNum = 1;
    ...
    async checkAll(event) {
      try {
        let isAll = event.target.checked ? "1" : "0";
        await this.$store.dispatch("isAllCheckCart",isAll);
        this.getDate()
      } catch (error) {
        alert(error.message);
      }
  ...
  computed: {
    ...
    isAllCheck() {
      //遍历数组全部符合条件则返回true，有一个不符合条件则返回false
